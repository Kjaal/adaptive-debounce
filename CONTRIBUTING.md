...
Pull requests targeting `main` run the same core and framework checks in GitHub Actions. Direct
pushes do not trigger workflows. Maintainers can also start CI manually from the Actions page. The
separate npm workflow requires a manual start and defaults to package-only; see
