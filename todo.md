...
- [x] `PERSIST-003` Bound background error handlers for coalesced autosaves (#5). Verified 500 autosave ticks behind a stalled custom adapter report two failures for two writes; preserved latest-state coalescing, manual save/flush rejection, disposal, and throwing-error-callback isolation. `HV-ADAPTIVE-DEBOUNCE-PERSIST-AUTOSAVE-HANDLERS` is `FIXED`; non-browser core checks pass.
