...
- [ ] `RELEASE-005` Require existing CI before changes reach `main` ([#9](https://github.com/Kjaal/adaptive-debounce/issues/9)).
  - [x] Prepare a native main-only ruleset requiring PRs and the verified GitHub Actions `check` result, with no permanent bypass, plus one-time activation and verification instructions.
  - [ ] After review and merge, activate the ruleset, enable native Dependabot alerts/security updates, and verify required-check enforcement on a PR. Checked-in policy does not activate live settings.
  - `HV-ADAPTIVE-DEBOUNCE-GITHUB-REQUIRED-CI` — `PENDING_HUMAN`: policy prepared; live activation remains pending.
