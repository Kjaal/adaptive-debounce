...
- [x] `FRAMEWORK-004` Fix [#10](https://github.com/Kjaal/adaptive-debounce/issues/10): clearing persistence during Vue startup discards the pending profile while preserving observation without another manual start. Deterministic tests cover either clear/load settlement order, subsequent typing, listener cleanup, and stop/dispose races. `HV-ADAPTIVE-DEBOUNCE-VUE-CLEAR-OBSERVATION` is `FIXED`.
